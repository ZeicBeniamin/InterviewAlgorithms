...
class Node {
   ...
   public:
    ...
    static shared_ptr<Node> makeNode() { return make_shared<Node>(); }

    static shared_ptr<Node> makeNode(int id) { return make_shared<Node>(id); }
};
...
int main() {
    ...
    if (found != nullptr) {
        cout << "Found node with id " << found->id << "at memory address "
             << found.get();
    }
}