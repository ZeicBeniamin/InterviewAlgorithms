...
std::priority_queue<
    std::pair<int, int>, vector<std::pair<int, int>>,
...
void find_path(int source, int num_nodes) {
    ...
    while (!node_q.empty()) {
        ...
        if (iter > 15461 - z && iter < 15461 + z) {
            cout << "Node " << currentNode.first << "   \t\t"
                 << distances[currentNode.first];
        }
        ...
        if (last_weight > distances[currentNode.first]) {
            cout << "Storing! " << "Last w: " << last_weight
                 << " curr: " << distances[currentNode.first] << "\n";
        }
        ...
        for (int i = 0; i < adjacent[currentNode.first].size(); ++i) {
            ...
            int potential_next_distance =
                distances[currentNode.first] + edgeWeight;
